...
/**
 * NeuralDoc API Route Registration
 * 
 * Registers all API endpoints for the NeuralDoc document AI platform.
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/documents/extract-url", async (req, res) => {
    ...
    try {
      ...
      console.log(`NeuralDoc: URL extraction request: ${url}`);
      ...
      console.log(`NeuralDoc: URL extraction initiated in ${extractTime}ms`);

      res.json(document);
    } catch (error) {
      const extractTime = Date.now() - extractStartTime;
      console.log(`NeuralDoc: URL extraction failed after ${extractTime}ms:`, error instanceof Error ? error.message : 'Unknown error');
      res.status(500).json({ message: error instanceof Error ? error.message : 'Unknown error' });
    }
  });
   ...
   * NeuralDoc document processing pipeline.
   * 
  ...
  app.post("/api/documents/upload", upload.single("file"), async (req, res) => {
    ...
    console.log(`NeuralDoc: File upload received: ${originalname} (${mimetype}, ${size} bytes)`);
    ...
    try {
      ...
      console.log(`NeuralDoc: Upload handled in ${uploadTime}ms`);
      res.json(document);
